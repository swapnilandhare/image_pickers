...
public class SelectPicsActivity extends BaseActivity {
    ...
    private void startSel() {
        ...
        // Null check to prevent NullPointerException in PictureStyleUtil.setStyle()
        if (uiColor == null) {
            uiColor = new HashMap<>();
        }
        ...
        if (mimeType != null) {
            ...
            PictureSelector.create(this)
                    .openCamera("photo".equals(mimeType) ? SelectMimeType.ofImage() : SelectMimeType.ofVideo())
                    ...
                    .setCropEngine(
                            (enableCrop)
                                    ? new ImageCropEngine(this, buildOptions(selectorStyle), width.intValue(),
                                            height.intValue())
                                    : null)
                            ...
                            if (result != null && result.size() > 0) {
                                ...
                                if ("video".equals(mimeType)) {
                                    ...
                                    if (videoDuration < videoRecordMinSecond.intValue()
                                            || videoDuration > videoRecordMaxSecond.intValue()) {
                                        String tips = "";
                                        if (videoDuration < videoRecordMinSecond.intValue()) {
                                            tips = getString(com.luck.picture.lib.R.string.ps_select_video_min_second,
                                                    videoRecordMinSecond.intValue());
                                        } else if (videoDuration > videoRecordMaxSecond.intValue()) {
                                            tips = getString(com.luck.picture.lib.R.string.ps_select_video_max_second,
                                                    videoRecordMaxSecond.intValue());
                                        }
                                        RemindDialog tipsDialog = RemindDialog.buildDialog(SelectPicsActivity.this,
                                                tips);
            ...
            PictureSelector.create(this).openGallery(selectMimeType)
                    ...
                    .setSelectionMode(selectCount.intValue() == 1 ? SelectModeConfig.SINGLE : SelectModeConfig.MULTIPLE)// 多选
                                                                                                                        // or
                                                                                                                        // 单选
    ...
    private void handlerResult(ArrayList<LocalMedia> selectList) {
        ...
        for (int i = 0; i < selectList.size(); i++) {
            ...
            if (localMedia.getMimeType().contains("image")) {
                ...
                Bitmap bitmap = ThumbnailUtils.createVideoThumbnail(localMedia.getAvailablePath(),
